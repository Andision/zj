...
//	}
	...
	$("#jp_home_menu a.menu-box").unbind("click").click(function(){
		...
		if(act=="jp/serviceapplication"){
			//如果点击服务，判断当前登录人返回原生首页或者H5首页
			Util.ajax({
				url : contextpath + "/jp/mcode/isPrimordial",//后台的请求路径
				success :function(r){
					if(r.primordial=='yes'){
						//返回原生
						wx.miniProgram.redirectTo({url: '/pages/service/service?openid='+r.openid})
					}else{
